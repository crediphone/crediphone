...
ALTER TABLE public.ordenes_reparacion
  ...
  ADD COLUMN IF NOT EXISTS tipo_firma VARCHAR(20),
  ADD COLUMN IF NOT EXISTS fecha_firma TIMESTAMP WITH TIME ZONE;

-- Agregar constraint si no existe (solo si la columna ya existe)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.ordenes_reparacion
    ADD CONSTRAINT ordenes_reparacion_tipo_firma_check
    CHECK (tipo_firma IN ('manuscrita', 'digital'));
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Usuarios autenticados pueden ver imágenes de reparaciones" ON public.imagenes_reparacion;
DROP POLICY IF EXISTS "Usuarios autenticados pueden crear imágenes" ON public.imagenes_reparacion;
DROP POLICY IF EXISTS "Usuarios autenticados pueden eliminar imágenes" ON public.imagenes_reparacion;
...
DROP POLICY IF EXISTS "Acceso público para validar tokens QR" ON public.sesiones_fotos_qr;
...
-- Nota: Las políticas de storage no soportan DROP IF EXISTS en todas las versiones
-- Si falla, ejecutar manualmente en Supabase Dashboard

DO $$
BEGIN
  -- Intentar eliminar políticas de storage si existen
  BEGIN
    DROP POLICY IF EXISTS "Usuarios autenticados pueden ver imágenes de reparaciones" ON storage.objects;
  EXCEPTION WHEN undefined_object THEN
    NULL; -- Política no existe, continuar
  END;

  BEGIN
    DROP POLICY IF EXISTS "Usuarios autenticados pueden subir imágenes de reparaciones" ON storage.objects;
  EXCEPTION WHEN undefined_object THEN
    NULL;
  END;

  BEGIN
    DROP POLICY IF EXISTS "Upload público durante sesiones QR" ON storage.objects;
  EXCEPTION WHEN undefined_object THEN
    NULL;
  END;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠ Advertencia: Bucket "reparaciones" no encontrado';
  END IF;
